...
usage() {  
    echo "Usage: $0 --benchmark BENCHMARKS --log_dir DIR --target TARGET"  
    echo ""  
    echo "Commands:"  
...
python inference.py \
    ...
    --fine_tuning \
    --fine_tuning_size 0.1 \
    --freeze_up_to 6 \
    --hidden_size 256 \
    --history_filename "history_train.yml" \
    --inference_benchmarks "$benchmark" \
    --learning_rate 0.0001 \
    --learning_rate_reduction 0.1 \
    --metrics_filename "metrics_train.yml" \
    --model_dir "$exe_path/models" \
    --model_name "$model_name" \
    --num_epochs 100 \
    --num_layers 12 \
    --output_dir "$log_directory" \
    --patience 200 \
    --predictions_filename "predictions_train.npz" \
    --random_state 42 \
...
python visualization.py \
    ...
    --plot_predictions \
    --plot_residuals \
    --predictions "$log_directory/predictions_train.npz" \
    --show_metrics "MAPE,R2,Shapiro" \
    --target $target
